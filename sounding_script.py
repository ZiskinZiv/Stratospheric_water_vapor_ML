...
def proccess_sounding_json(savepath, igra_id='BRM00082332'):
    import pandas as pd
    import json
    import xarray as xr
    # loop over lines lists in each year:
    pw_years = []
    for file in savepath.glob(igra_id + '*.json'):
...
def get_sounding_data_wyoming(savepath, start_date='2005-01-01',
                              end_date='2019-04-30', igra_id='ISM00040179'):
    """get upper air soundings from wyoming uni. st_id for bet dagan is 40179"""
    import requests
    ...
    for year in years:
        ...
        for date in time.sel(time=str(year)).values:
            ...
            print('getting {} station datetime: {}'.format(igra_id,
            ...
            url = ('http://weather.uwyo.edu/cgi-bin/sounding?region=mideast&'
                   ...
                   '&FROM=' + day + hour + '&TO=0100&STNM=' + st_id)
            r = requests.get(url)
            soup = bs(r.text, "lxml")
    ...
    #        dict_list.append(dict(zip(keys, values)))
        ...
        filename = igra_id + '_soundings_' + str(year) + '.json'
        with open(savepath / filename, 'w') as fout:
            json.dump(lines_list, fout)
        print('Done!')
    return

get_sounding_data_wyoming(Path('/home/ziskin/Work_Files/Stratosphere_Chaim/sounding/'),
                          start_date='1993-01-01', igra_id='BRM00082332')
get_sounding_data_wyoming(Path('/home/ziskin/Work_Files/Stratosphere_Chaim/sounding/'),
                          start_date='1993-01-01', igra_id='RMM00091376')
